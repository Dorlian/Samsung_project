...
Если нужен один `.exe` без установленного Python, рекомендуется **Python 3.10–3.12** (у **3.14** часто нет готовых колёс **torch** и больше предупреждений PyInstaller). Сборочные bat и команда `pyinstaller …` должны выполняться из **корня репозитория** (рядом с папкой **`app/`**).
