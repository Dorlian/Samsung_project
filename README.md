...
**Опциональная нейросеть для глаз:** при включённой проверке и наличии **`models/eye_state_resnet18.pth`** он подхватывается автоматически. Без файла используется только MediaPipe (blendshapes). Обучить и сохранить веса можно скриптом `src/training/train_eye_classifier.py`.
